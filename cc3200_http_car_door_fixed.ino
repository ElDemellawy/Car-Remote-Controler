/**
 * CC3200 Car Door Controller - HTTP Version
 * 
...
void connectToWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.print("WiFi.status() error code: ");
    Serial.println(WiFi.status());
    wifiConnected = false;
  }
}
...
void sendStatusUpdate() {
  ...
  if (client.connect(serverIP.c_str(), serverPort)) {
    ...
    // Create JSON payload
    String jsonPayload = "{";
    jsonPayload += "\"device\":\"car_door\",";
    jsonPayload += "\"status\":\"";
    jsonPayload += (carDoorUnlocked ? "unlocked" : "locked");
    jsonPayload += "\",";
    ...
    client.println("POST /api/car_door/status HTTP/1.1");
...
void handleHttpRequests() {
  ...
  if (request.indexOf("GET /") >= 0) {
    // Status request
    response = "{";
    response += "\"device\":\"car_door\",";
    response += "\"status\":\"";
    response += (carDoorUnlocked ? "unlocked" : "locked");
    response += "\",";
    response += "\"timestamp\":";
    response += millis();
    response += "}";
  } else if (request.indexOf("POST /unlock") >= 0) {
    carDoorUnlocked = true;
    Serial.println("Car door UNLOCKED via HTTP");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Car Door Status:");
    lcd.setCursor(0, 1);
    lcd.print("UNLOCKED");
    response = "{";
    ...
    response += "}";
    sendStatusUpdate();
  } else if (request.indexOf("POST /lock") >= 0) {
    carDoorUnlocked = false;
    Serial.println("Car door LOCKED via HTTP");
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Car Door Status:");
    lcd.setCursor(0, 1);
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n=== CC3200 HTTP Car Door Controller ===");
  ...
  if (wifiConnected) {
    ...
    Serial.println("  POST /unlock    - Unlock car door");
    Serial.println("  POST /lock   - Lock car door");
    Serial.println();
